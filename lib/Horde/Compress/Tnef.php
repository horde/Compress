...
class Horde_Compress_Tnef extends Horde_Compress_Base
{
    ...
    public const MAPI_START_DATE                   = 0x0060; // pidTag
    public const MAPI_END_DATE                     = 0x0061; // pidTag
    public const MAPI_APPOINTMENT_SEQUENCE         = 0x8201; // pidLid
    public const MAPI_BUSY_STATUS                  = 0x8205; // pidLid
    public const MAPI_MEETING_REQUEST_TYPE         = 0x0026; // pidLid
    public const MAPI_RESPONSE_REQUESTED           = 0x0063; // pidTag
    public const MAPI_APPOINTMENT_LOCATION         = 0x8208; // pidLid
    public const MAPI_APPOINTMENT_URL              = 0x8209; // pidLid
    public const MAPI_APPOINTMENT_START_WHOLE      = 0x820D; // Full datetime of start (FILETIME format)
    public const MAPI_APPOINTMENT_END_WHOLE        = 0x820E; // Full datetime of end (FILETIME format)
    public const MAPI_APPOINTMENT_DURATION         = 0x8213; // pidLid - duration in minutes.
    public const MAPI_APPOINTMENT_SUBTYPE          = 0x8215; // (Boolean - all day event?)
    public const MAPI_APPOINTMENT_RECUR            = 0x8216; // This seems to be a combined property of MAPI_RECURRING, MAPI_RECURRING_TYPE etc...?
    public const MAPI_APPOINTMENT_STATE_FLAGS      = 0x8217; // (bitmap for meeting, received, cancelled?)
    public const MAPI_RESPONSE_STATUS              = 0x8218;
    public const MAPI_RECURRING                    = 0x8223;
    public const MAPI_RECURRENCE_TYPE              = 0x8231;
    public const MAPI_ALL_ATTENDEES                = 0x8238; // ALL attendees, required/optional and non-sendable.
    public const MAPI_TO_ATTENDEES                 = 0x823B; // All "sendable" attendees that are REQUIRED.

    // tz. Not sure when to use STRUCT vs DEFINITION_RECUR. Possible ok to always use STRUCT?
    public const MAPI_TIMEZONE_STRUCT              = 0x8233; // Timezone for recurring mtg?
    public const MAPI_TIMEZONE_DESCRIPTION         = 0x8234; // Description for tz_struct?
    public const MAPI_START_CLIP_START             = 0x8235; // Start datetime in UTC
    public const MAPI_START_CLIP_END               = 0x8236; // End datetime in UTC
    public const MAPI_CONFERENCING_TYPE            = 0x8241;
    public const MAPI_ORGANIZER_ALIAS              = 0x8243; // Supposed to be organizer email, but it seems to be empty?
    public const MAPI_APPOINTMENT_COUNTER_PROPOSAL = 0x8257; // Boolean
    public const MAPI_TIMEZONE_START               = 0x825E; // Timezone of start_whole
    public const MAPI_TIMEZONE_END                 = 0x825F; // Timezone of end_whole
    public const MAPI_TIMEZONE_DEFINITION_RECUR    = 0x8260; // Timezone for use in converting meeting date/time in recurring meeting???
    public const MAPI_REMINDER_DELTA               = 0x8501; // Minutes between start of mtg and overdue.
    public const MAPI_SIGNAL_TIME                  = 0x8502; // Initial alarm time.
    public const MAPI_REMINDER_SIGNAL_TIME         = 0x8560; // Time that item becomes overdue.
    public const MAPI_ENTRY_UID                    = 0x0003; // pidLidGlobalObjectId, PSETID_MEETING
    public const MAPI_ENTRY_CLEANID                = 0x0023; // pidLidCleanGlobalObjectId, PSETID_MEETING
    public const MAPI_MEETING_TYPE                 = 0x0026; // pidLidMeetingType, PSETID_MEETING
    ...
    public const IPM_TASK_GUID                     = 0x8519; // pidLidTaskGlobalId, PSETID_Common
            ...
            case self::AMCLASS:
                ...
                switch ($message_class) {
                    ...
                    case self::IPM_TASK_REQUEST:
                        $this->_currentObject = new Horde_Compress_Tnef_VTodo($this->_logger, null, ['parent' => &$this]);
                        $this->_files[] = $this->_currentObject;
                        break;
                    default:
